...
class Coordinate(NamedTuple):
    ...
    @classmethod
    def random(
        cls, range_x0: int, range_x1: int, range_y0: int, range_y1: int
    ) -> "Coordinate":
        return cls(
            random.randint(range_x0, range_x1), random.randint(range_y0, range_y1)
        )
...
class TRONAction(NamedTuple):
    ...
    def create(
        ...
        if player_1_selector not in DIRECTIONS.SELECTORS:
            raise RuntimeError(
                f"Invalid action selector for player 1: {player_1_selector}"
            )
        if player_2_selector not in DIRECTIONS.SELECTORS:
            raise RuntimeError(
                f"Invalid action selector for player 2: {player_2_selector}"
            )
