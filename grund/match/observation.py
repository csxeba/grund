...
from .config import LearningType, MatchConfig, ObservationType
...
class MatchObservationMaker:
    ...
    def get_pixel_observation(self, ball, team1, team2):
        ...
        perspective_base = cv2.circle(
            perspective_base, ball.position[::-1], ball.radius, self.cfg.ball_color, 2
        )
    ...
    def get_numeric_observation(
        self, ball: Ball, team1: List[Player], team2: List[Player]
    ):
        """
        ...
        for i, player in enumerate(team1 + team2):
            ...
            for j, mate in enumerate(
                p for p in ego_team[player.team] if p.ID != player.ID
            ):
                idx = j * 2
